     ...
     <section className="relative overflow-hidden py-16">
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold text-dark md:text-4xl">Работаем с надёжными лизинговыми компаниями</h2>
        ...
        <ul className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {partners.map((src, index) => (
            <li
              key={`partner-${index}`}
              className="flex h-28 items-center justify-center rounded-3xl border border-white/60 bg-white/80 px-6 py-4 shadow-glow backdrop-blur transition hover:border-white"
            >
              <Image
                src={src}
