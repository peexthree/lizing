...
// Выносим ModalContent в отдельный компонент
const ModalContent = ({
  status,
  closeModal,
  onSubmit,
  ...
  form: FormState
  handleChange: (field: keyof FormState) => (event: ChangeEvent<HTMLInputElement>) => void
...
export default function LeadForm({ variant = 'default', className }: LeadFormProps) {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!agree || name.length < 2 || phoneDigits.length < 11) {
      setStatus('err')
